...
def fun_estimate_parameters(x, read_num_seq, t_seq, kappa=2.5, fitness_type='m'):
    ...
    if fitness_type == 'w':
        for i in range(1, seq_num):
            x_mean[i] = np.max(
                    np.dot(x, read_num_seq[:, i]) / read_depth_seq[i]
                    , 0)
            read_num_est_tempt = np.exp(
                    (t_seq[i] - t_seq[i - 1]) * (np.log(1 + x) + 1) 
                        - (t_seq[i] - t_seq[i - 1]) 
                        / (x_mean[i] - x_mean[i - 1]) 
            ...
            read_num_est_tempt = (
                    read_num_est_tempt * read_num_seq[:, i - 1] 
                        / read_depth_seq[i - 1] * read_depth_seq[i]
                    )
            read_num_seq_est[:, i] = read_num_est_tempt
            x_mean[i] = (
                    np.dot(x, read_num_seq_est[:, i]) 
                        / np.sum(read_num_seq_est[:, i])
                    )
        ...
        for i in range(1, seq_num):
            ...
            read_num_est_tempt = ( read_num_est_tempt 
                    * read_num_seq[:, i - 1] / read_depth_seq[i - 1] 
                    * read_depth_seq[i]
                    )
            read_num_seq_est[:, i] = read_num_est_tempt
            x_mean[i] = (
    ...
    likelihood_log_seq[pos1_r, pos1_c + 1] = (
            0.25 * np.log(read_num_seq_est[pos1_r, pos1_c + 1])
                - 0.5 * np.log(4 * np.pi * kappa)
                - 0.75 * np.log(read_num_seq[pos1_r, pos1_c + 1])
                - (
                    np.sqrt(read_num_seq[pos1_r, pos1_c + 1])
                        - np.sqrt(read_num_seq_est[pos1_r, pos1_c + 1])
                    ) 
            ** 2 / kappa
            )
    ...
    likelihood_log_seq[pos2_r, pos2_c + 1] = (
            np.multiply(read_num_seq[pos2_r, pos2_c + 1],
                ...
                + read_num_seq[pos2_r, pos2_c + 1] - 0.5 
                * np.log(2 * np.pi * read_num_seq[pos2_r, pos2_c + 1])
            )

    factorial_tempt = [float(math.factorial(i)) 
            for i in read_num_seq[pos3_r, pos3_c + 1].astype(int)
            ]

    likelihood_log_seq[pos3_r, pos3_c + 1] = (
            np.multiply(read_num_seq[pos3_r, pos3_c + 1],
...
def fun_likelihood_lineage_w(x):
    ...
    for i in range(1, seq_num):
        read_num_lineage_est_tempt = np.exp(
                (t_seq_global[i] - t_seq_global[i - 1]) * (np.log(1 + x) + 1) 
                    - (t_seq_global[i] - t_seq_global[i - 1]) 
                    / (x_mean_global[i] - x_mean_global[i - 1]) 
                    * ((x_mean_global[i] + 1) * np.log(x_mean_global[i] + 1)
                    - (x_mean_global[i - 1] + 1) 
                    * np.log(x_mean_global[i - 1] + 1))
                )

        read_num_seq_lineage_est[i] = (
                read_num_lineage_est_tempt * read_num_seq_lineage_global[i - 1]
    ...
    likelihood_log_seq_lineage[pos2 + 1] = (
            np.multiply(read_num_seq_lineage_global[pos2 + 1],
                ...
                + read_num_seq_lineage_global[pos2 + 1] - 0.5 
                * np.log(2 * np.pi * read_num_seq_lineage_global[pos2 + 1])
            )

    factorial_tempt = [float(math.factorial(i)) 
            for i in read_num_seq_lineage_global[pos3 + 1].astype(int)
            ]
    
    likelihood_log_seq_lineage[pos3 + 1] = (
            np.multiply(read_num_seq_lineage_global[pos3 + 1],
...
def fun_likelihood_lineage_m(x):
    ...
    for i in range(1, seq_num):
        read_num_lineage_est_tempt = np.exp(
                (t_seq_global[i] - t_seq_global[i - 1]) * x 
                    - (t_seq_global[i] - t_seq_global[i - 1]) * 
    ...
    pos1 = np.where(read_num_seq_lineage_global[:-1] >= 20)[0]
    likelihood_log_seq_lineage[pos1 + 1] = (
            0.25 * np.log(read_num_seq_lineage_est[pos1 + 1]) - 0.5 
                * np.log(4 * np.pi * kappa_global) - 0.75 
                * np.log(read_num_seq_lineage_global[pos1 + 1]) 
                - (np.sqrt(read_num_seq_lineage_global[pos1 + 1]) 
                    - np.sqrt(read_num_seq_lineage_est[pos1 + 1])
                    ) ** 2 
            / kappa_global
...
def main():
    ...
    -- kappa: a noise parameter that characterizes the total noise introduced by 
                ...
                [S. F. Levy et al. Quantitative Evolutionary Dynamics Using 
                High-resolution Lineage Tracking. Nature, 519: 181–186 (2015)].) 
    -- regression_num: number of points used in the initial 
                linear-regression-based fitness estimate (default: 2)
    -- fitness_type: type of fitness: Wrightian fitness (w), or Malthusian 
                fitness (m) (default: m)
    ...
    output_filename_FitSeq_Result.csv: 
        ...
        3rd column: estimated mean fitness per sequenced time-point 
            [x_mean(0), x_mean(t1), ...]
        4th column+: estimated reads number per genotype per sequencing 
    ...
    if fitness_type == 'w':
        if regression_num == 2:
            x0_tempt = np.power(np.true_divide(read_freq_seq[:, 1], 
                    read_freq_seq[:, 0]), 1 / (t_seq[1] - t_seq[0])) - 1
        else:
            x0_tempt = [regression_output.slope 
                    for i in range(lineages_num) for regression_output in
                        [   linregress(t_seq[0:regression_num], 
        ...
        if regression_num == 2:
            x0_tempt = np.true_divide(read_freq_seq[:, 1] - 
                    read_freq_seq[:, 0], t_seq[1] - t_seq[0])
        else:
            x0_tempt = [regression_output.slope 
                    for i in range(lineages_num) for regression_output in
                        [   linregress(t_seq[0:regression_num], 
    ...
    t_seq_global = t_seq
    kappa_global = kappa
    parameter_output = fun_estimate_parameters(
                                x0, read_num_seq, t_seq, kappa, fitness_type)
    x_mean_global = parameter_output['Estimated_Mean_Fitness']
    likelihood_log_sum_iter = [-1e50 * lineages_num, 
                                    np.sum(parameter_output['Likelihood_Log'])]
    ...
    while ( 
            ...
            (
                iter_num <= min_iter
                or 
                (likelihood_log_sum_iter[-1] - likelihood_log_sum_iter[-2] 
                    >= step_size)
                )
            ):
        ...
        if fitness_type == 'w':
            ...
            pos = (
                    np.true_divide(read_num_seq[:, 0] 
                        / np.sum(read_num_seq[:, 0], axis=0), 
                    np.sum(read_num_seq[:, 1], axis=0)) 
                        > 2 ** (t_seq[1] - t_seq[0])
                    ) | (x_opt <= -1)
            x_opt[pos] = x0[pos]
        elif fitness_type == 'm':
        ...
        parameter_output = fun_estimate_parameters(
                x_opt, read_num_seq, t_seq, kappa, fitness_type)
        likelihood_log_sum_iter.append(
                np.sum(parameter_output['Likelihood_Log'])
                )
        x_mean_global = parameter_output['Estimated_Mean_Fitness']
        iter_num += 1
        print('         likelihood value (log): %.4f' 
                %likelihood_log_sum_iter[-1])

    second_derivative = np.zeros(lineages_num)
    if fitness_type == 'w':
        for i in range(lineages_num):
            second_derivative[i] = derivative(fun_likelihood_lineage_w, 
                    x_opt[i], dx=1e-6, n=2)
    elif fitness_type == 'm':
        for i in range(lineages_num):
            second_derivative[i] = derivative(fun_likelihood_lineage_m, 
                    x_opt[i], dx=1e-6, n=2)
    estimation_error = 1/np.sqrt(second_derivative)

    read_num_seq_est = parameter_output['Estimated_Read_Number']
    x_opt = (
            x_opt - np.dot(read_num_seq_est[:, 0], x_opt) 
                / np.sum(read_num_seq_est[:, 0])
            )
    parameter_output_final = fun_estimate_parameters(
                                x_opt, read_num_seq, t_seq, kappa, fitness_type)
    ...
    for i in range(read_num_seq_est.shape[1]):
        fitseq_output['Estimated_Read_Number_t%d' % i] = (
                read_num_seq_est[:, i].astype(int)
                )
